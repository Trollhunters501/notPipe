...
Рабочий клиент YouTube для **Android 1.5+**, использующий несколько серверных API. Создан с ❤ и расчётом на долговечность. Вместо того чтобы зависеть от одного инстанса, который может внезапно перестать работать, приложение параллельно использует сразу несколько для максимальной надежности.
...
* [Invidious](https://docs.invidious.io/api/) (метаданные, 360p)
* [yt2009](https://github.com/ftde0/yt2009) (все качества, конвертация, каналы)
* [Piped](https://docs.piped.video/docs/api-documentation/) (метаданные, 360p)
* [YtAPILegacy](https://github.com/ZendoMusic/yt-api-legacy) (метаданные, все качества, конвертация)
...
## Возможности
> [!NOTE]  
> Приложение не подключается к YouTube напрямую, поэтому это обходит различные сетевые ограничения. См. «[Зачем и как это работает](#зачем-и-как-это-работает)».
* Популярное, поиск
* Видео, похожие видео, комментарии
* Воспроизведение видео в 360p/480p/720p/1080p
* Каналы
* Конвертация видео для устройств без поддержки H.264 в кодеки MPEG-4 Visual и H.263
* Автоматическое обновление списка инстансов с ссылки
* Планшетный интерфейс
### TODO
* Скачивание видео
* Скачивание аудио
* Авторизация в YouTube-аккаунт через YTFixer/yt2009 (рекомендации, подписки, история и т. д.)
* Плейлисты
* Локальный аккаунт для подписок (как в NewPipe)

## Исправляем проблемы на Android ≤2.3
...
* **Единая точка отказа.** Предыдущие способы смотреть YouTube полагаются на один конкретный инстанс API (сервер). Как только такой инстанс получает заметный объём трафика, YouTube автоматически банит его IP-адрес. Кроме того, один сервер может быть сильно перегружен, из-за чего им становится невозможно пользоваться.
...
## Сообщить о баге
**Вам во вкладку «[Issues](https://github.com/gohoski/notPipe/issues)» на GitHub!** Не забудьте указать, на какой версии Android и архитектуре (ARMv5/v6/v7/др.) вы столкнулись с проблемой.
...
* **Отдельное спасибо всем контрибьюторам [Invidious](https://github.com/iv-org/invidious/graphs/contributors?all=1), [yt2009](https://github.com/ftde0/yt2009/graphs/contributors?all=1), [Piped](https://github.com/TeamPiped/Piped-Backend/graphs/contributors?all=1) и [YtAPILegacy](https://github.com/ZendoMusic/yt-api-legacy/graphs/contributors?all=1) за создание этих потрясающих API**
...
* [ReOldTube от YMP Yuri](https://github.com/YMP-CO/ReOld-Tube) — клиент для Android 3.0+, использующий Invidious. Хоть и в настройках его можно сконфигурировать для работы и сегодня, в нём полно багов *(да и написан он «на вайбах» / vibecoded)*.
* [YT-Legacy-Eclair от MoreVista](https://github.com/MoreVista/YT-Legacy-Eclair) — простой клиент для Android 2.1+, использующий Invidious. Единичный инстанс можно сменить в настройках.
## Лицензия
